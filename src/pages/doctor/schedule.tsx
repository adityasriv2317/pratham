...
const ScheduleCalendar: React.FC = () => {
  ...
  const modalAppointments = selectedDate
    ? apptMap[selectedDate.toISOString().slice(0, 10)] || []
    : [];

  return (
    <Layout role="doctor">
      <div className="min-h-screen rounded-2xl bg-white py-10 px-2 sm:px-8 flex flex-col items-center">
        <h1 className="text-3xl font-bold text-blue-900 mb-6 text-center">
          My Schedule
        </h1>
        <div className="bg-blue-50 rounded-2xl shadow-lg p-6 w-full max-w-2xl">
          <div className="flex justify-between items-center mb-4">
            <button
              className="text-blue-500 hover:text-blue-900 text-xl font-bold"
              onClick={() =>
                setCurrentMonth((m) => {
          ...
          <div className="grid grid-cols-7 gap-2 text-center mb-2">
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
              <div key={d} className="font-semibold text-blue-700">
                {d}
              </div>
            ))}
          </div>
          <div className="grid grid-cols-7 gap-3">
            ...
            {days.map((date) => {
              ...
              return (
                <button
                  key={key}
                  className={`rounded-lg h-15 flex flex-col items-center justify-center border transition-all
                    ${
                      appts.length > 0
        ...
        {showModal && selectedDate && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 bg-opacity-40">
            <div className="bg-white rounded-2xl shadow-xl px-8 py-12 w-full max-w-lg relative">
              <button
                className="absolute top-4 right-4 text-blue-500 hover:text-blue-700 border-2 rounded-lg text-2xl font-bold"
                onClick={() => setShowModal(false)}
                aria-label="Close"
              >
                <X className="w-7 h-7" />
              </button>
              <h2 className="text-xl font-bold text-blue-900 mb-4 text-center flex items-center gap-2 justify-center">
                <CalendarDays className="w-6 h-6 text-blue-700" /> Appointments
              ...
              {modalAppointments.length === 0 ? (
                ...
                <div className="flex flex-col gap-3">
                  {modalAppointments.map((appt) => (
                    <div
                      key={appt._id}
                      className="rounded-lg border border-blue-200 bg-blue-50 p-3 flex flex-col gap-1"
                    >
